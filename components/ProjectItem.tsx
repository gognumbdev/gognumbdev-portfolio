...
const ProjectItem = ({image,title,skills,projectUrl}:projectProps) => {
  ...
  return (
    <div className="relative flex items-center justify-center h-auto w-full shadow-xl shadow-gray-400 
        ...
        <Image className='rounded-xl group-hover:opacity-10' src={image} alt="/" objectFit="contain" />
        <div className='hidden group-hover:block absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]'>
            <h3 className='text-2xl text-white tracking-wider text-center'>{title}</h3>
            {skills.map(skill => <p key={skill} className='pb-4 pt-2 text-white text-center'>{skill}</p>)}
